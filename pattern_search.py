...
def brute_force(domains):
    ...
    def backtrack():
        ...
        # Seleciona as variáveis não atribuídas, ordenadas por tamanho do domínio (menor primeiro)
        unassigned = [v for v in domains if v not in assignment]
        var = min(unassigned, key=lambda v: (len(domains[v]), v))  # desempate pelo nome

        # Tenta os valores de forma ordenada para consistência (diferencial deste código)
...
for i in range(1, 11):
    ...
    test_instance(twenty_rules_without_solution, "Instância de teste - Einstein simplificado")
    print("\n")